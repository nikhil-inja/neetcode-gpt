...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.posEmbed = nn.Embedding(context_length, model_dim)
        self.block = nn.Sequential()
        for i in range(num_blocks):
            self.block.append(self.TransformerBlock(model_dim, num_heads))
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        tokens = self.tokenEmbed(context)
        positions = self.posEmbed(torch.arange(context.shape[1], device=context.device))
        h = tokens + positions
        h = self.block(h)
        logits = self.norm(h)
